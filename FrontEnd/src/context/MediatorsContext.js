...
export const MediatorsProvider = ({ children }) => {
  ...
  const getMediatorById = async (id) => {
    try {
      ...
      return response; // Response is already the mediator object
    } catch (err) {
      setError(err.message);
      throw err;
    } finally {
  ...
  const createMediator = async (mediatorData) => {
    try {
      ...
      setMediators(prev => [...prev, response]); // Response is already the mediator object
      return response;
    } catch (err) {
      setError(err.message);
      throw err;
  ...
  const updateMediator = async (id, mediatorData) => {
    try {
      ...
      setMediators(prev => 
        prev.map(mediator => 
          mediator._id === id 
            ? { ...mediator, ...response } // Response is already the updated mediator object
            : mediator
        )
      );
      return response;
